...
from .wavelet import forward_cwt, inverse_cwt
...
class Block(object):
    ...
    def single_process(self):
        data_out, scales = forward_cwt(
                self.data, self.params.wave_type, self.params.nvoices, self.dt
                )
        icwt = inverse_cwt(data_out, self.params.wave_type, self.params.nvoices)
    ...
    def batch_process(self):
        ...
        for arr in cwt:
            icwt.append(
                inverse_cwt(arr, self.params.wave_type, self.params.nvoices)
               )
    ...
    def plot(self, trace_id=0):
        """
        Plot time-frequency representation (TRF) of a single time series.

        :param trace_id: time series to plot.
        :type trace_id: int
        """
        try:
            arr = self.data[trace_id]
            times = np.arange(0,len(arr),self.dt)
        except:
            try:
                arr = self.data[trace_id].data
                times = self.data[trace_id].times()
            except:
                print("Cannot read input data.")

        fig = plt.figure(figsize=(10,6.5))
        ax1 = fig.add_subplot(2,1,1)
        y_axis = np.log10(self.scales[trace_id])
        extent = [min(times), max(times), min(y_axis), max(y_axis)]
        ax1.imshow(abs(self.cwt[trace_id]), extent=extent, aspect="auto") # need to fix x-axis to time
        ax1.set_xlim([min(times), max(times)])
        ax1.set_ylim([min(y_axis), max(y_axis)])
        ax1.set_ylabel("Scale [log10(s)]")
        ax1.set_title("Time Frequency Representation (TRF)")

        ax2 = fig.add_subplot(2,1,2)
        ax2.plot(times, arr, color="black", linewidth=1)
        #ax2.plot(times, arr, "r--", linewidth=0.8)
