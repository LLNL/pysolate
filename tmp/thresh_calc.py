...
def noiseModel(Wx, delta, noise_model, noise_starttime, noise_endtime, nsigma_method, nlbound):
    ...
    :return S:
    : rtype S:
    :return P: threshold from empirical cdf of the noise signal at the desired confidence level
     :rtype P:
    ...
    newbeg = int(np.round(noise_starttime/delta))
    newend = int(np.round(noise_endtime/delta)+1)

    # donoho missing sigma factor input
...
def signalThresh(Wx, signal_threshold, P):
    ...
    W_test = np.abs(Wx)
    #Wx_new = copy.deepcopy(Wx)
    if signal_threshold == "hard":
        Wx_new = (Wx.T * (P>W_test.T)).T
    elif signal_threshold == "soft":
        #Wx_new = ((np.sign(Wx).T * P) * (P<=W_test.T) + W_test.T*(P>W_test.T)).T
