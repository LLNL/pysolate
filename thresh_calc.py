...
def signalThresh(Wx, signal_threshold, P):
    ...
    if signal_threshold == "soft":
        W_test=abs(Wx)
        Wx_new = np.transpose(np.sign(np.transpose(Wx))* P) * np.transpose(P <= np.transpose(W_test)) + np.transpose(np.transpose(W_test)*(P > np.transpose(W_test)))
        #Wx_new = np.transpose(np.sign(Wx))* P * (P <= np.transpose(W_test)) + np.transpose(W_test)*(P > np.transpose(W_test))
        #Wx_new = np.transpose(Wx_new)
    return Wx_new
